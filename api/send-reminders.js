...
// Automatically triggered every 2 minutes or via API request to send background phone notifications
  ...
  try {
    ...
    for (const sub of subscriptions) {
      ...
      try {
        ...
        results.push({ username: sub.username, endpoint: sub.endpoint.slice(0, 30), status: 'sent_success' });
      } catch (pushErr) {
        console.error('Web push error:', pushErr);
        // If subscription is expired or unsubscribed (410 / 404), clean it up from DB
